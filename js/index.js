...
function showError(error) {
    switch (error.code) {
        case error.PERMISSION_DENIED:
            devlog("User denied the request for Geolocation.");
            break;
        case error.POSITION_UNAVAILABLE:
            devlog("Location information is unavailable.");
            break;
        case error.TIMEOUT:
            devlog("The request to get user location timed out.");
            break;
        case error.UNKNOWN_ERROR:
            devlog("An unknown error occurred.");
            break;
    }
}
